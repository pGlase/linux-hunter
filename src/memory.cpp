...
std::wstring memory::browser::GetFormattedPartyMemberString()
{
    ...
    {
        std::wcout << "reading address: " << member.Address << "\n";
        std::wstring playerName = read_utf8(member.Address + 0x49, 32 + 1, true);
	    if((playerName.empty())) // || (playerName.find_first_of(L'\0') != std::wstring::npos))
         {
             playerName = L"N/A";
        }
        char weaponId = read_mem<char>(member.Address + 0x7C, true);
...
long memory::browser::ReadMemoryLongAt(size_t address, const std::vector<size_t>& offsets)
{
    ...
    for(const auto& offset : offsets)
    {
        ...
        long tempValue = 0;
        if(!this->direct_mem_read(currentAddress, reinterpret_cast<void*>(&tempValue), length))
        {
...
int memory::browser::ReadMemoryIntAt(size_t address, const std::vector<size_t>& offsets)
{
    size_t currentAddress = address;
    for(const auto& offset : offsets)
    {
...
int memory::browser::DerefMemory(size_t address, const std::vector<size_t>& offsets)
{
    long ptr = this->ReadMemoryLongAt(address, offsets);
    return this->ReadMemoryIntAt(ptr,{0});
}

std::vector<PartyMemberStructure> memory::browser::LoadPartyMembers()
{
    const auto partyInformationPointer = this->LoadPartyInformationPointer();
    if(partyInformationPointer == 0)
    {
        return{};
        //throw std::runtime_error("lol, the party pointer is bad");
    }
    constexpr long basePointer = 0x140000000;
    constexpr long sessionOffset = 0x051C2478;
    constexpr long sessionDataAddress = basePointer + sessionOffset;
    const std::vector<size_t> sessionPartyOffsets {0x258,0x10,0x6574};
    const int partyCount = this->DerefMemory(sessionDataAddress, sessionPartyOffsets);
...
size_t memory::browser::LoadPartyInformationPointer()
{
    constexpr long basePointer = 0x140000000;
    constexpr long questDataOffset = 0x050112F0;
    constexpr long questDataAddress = basePointer + questDataOffset;
    const std::vector<size_t> questDataOffsets{0x54};
    const long questInformation = ReadMemoryLongAt(questDataAddress, questDataOffsets);
    ...
    constexpr long partyAddressInfoPtr = basePointer + partyDataOffset;
    const std::vector<size_t> partyInfoOffsets{0x1AB0};
    const long partyInformationPointer = ReadMemoryLongAt(partyAddressInfoPtr, partyInfoOffsets);
    if(partyInformationPointer == 0)
    {
...
std::vector<PartyMemberStructure> memory::browser::ReadPartyMemberStructure(long address, int count)
{
    ...
    for(int i=0; i<count; i++)
    {
        ...
        direct_mem_read((address + i*structSize), reinterpret_cast<void*>(&data), structSize);
...
bool memory::browser::safe_load_effective_addr_rel(const size_t addr, size_t& out, const bool refresh) {
	constexpr int opcodeLength = 3;
	constexpr int paramLength = 4;
	constexpr int instructionLength = opcodeLength + paramLength;
...
size_t memory::browser::load_effective_addr_rel(const size_t addr, const bool refresh) {
	...
	if(!safe_load_effective_addr_rel(addr, out, refresh))
		throw std::runtime_error("Couldn't find specified address, rel");
	return out;
}
...
size_t memory::browser::load_multilevel_addr_rel(const size_t addr, const uint32_t* off_b, const uint32_t* off_e, const bool refresh) {
	...
	if(!safe_load_multilevel_addr_rel(addr, off_b, off_e, rv, refresh))
		throw std::runtime_error("Couldn't find specified address, multi");
	return rv;
}
